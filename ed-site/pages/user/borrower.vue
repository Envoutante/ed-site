  ...
  methods: {
    ...
    initSelected() {
      ...
      this.$axios
        ...
        .then((res) => {
          this.returnSourceList = res.data.dictList
        })

      // 联系人关系列表
      this.$axios.$get('/api/core/dict/findByDictCode/relation').then((res) => {
        this.contactsRelationList = res.data.dictList
      })
    },

    onUploadSuccess(res, file, fileType) {
      if (res.code === 0) {
        // 业务成功
    ...
    onUploadSuccessIdCard1(res, file) {
      this.onUploadSuccess(res, file, 'idCard1')
    },

    onUploadSuccessIdCard2(res, file) {
      this.onUploadSuccess(res, file, 'idCard2')
    },

    onUploadSuccessHouse(res, file) {
      this.onUploadSuccess(res, file, 'house')
    },

    onUploadSuccessCar(res, file) {
      this.onUploadSuccess(res, file, 'car')
    },
    ...
    onUploadRemove(file, fileList) {
      ...
      this.$axios
        ...
        .then((res) => {
          console.log('远程删除文件成功', file)
        })

      // 2. 删除borrowerAttachList中的相应对象
