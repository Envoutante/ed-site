  ...
  <div class="personal-main">
    <div class="personal-pay">
      ...
      <div v-if="active === 0" class="user-borrower">
        ...
        <el-alert
          title="您提供的任何信息易贷都承诺予以保护，不会挪作他用。"
          type="warning"
          :closable="false"
        >
        </el-alert>
  ...
  methods: {
      ...
      this.$axios
        ...
        .then((res) => {
          ...
          if (this.borrowInfoStatus === 0) {
            // 未认证时
            this.active = 0
            this.getBorrowAmount() // 获取借款额度
            this.initSelected() // 初始化下拉列表
    ...
    initSelected() {
      ...
      this.$axios
        ...
        .then((res) => {
          this.returnMethodList = res.data.dictList
        })

      // 资金用途列表
      this.$axios.$get('/api/core/dict/findByDictCode/moneyUse').then((res) => {
        this.moneyUseList = res.data.dictList
      })
    },
