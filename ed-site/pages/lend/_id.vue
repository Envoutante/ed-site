    ...
    <div class="item-detail-head clearfix" data-target="sideMenu">
      ...
      <div v-if="userType === 1 && lend.status === 1" class="bd clearfix">
        <div class="data" style="width: auto;">
          ...
          <p>
            您的账户余额为 <span class="c-orange">{{ account }}</span> 元，
...
export default {
  async asyncData({ $axios, params }) {
    ...
    let res = await $axios.$get('/api/core/lend/show/' + lendId) // 同步调用
    ...
    return {
      lend: res.data.lendDetail.lend, // 标的详情
      borrower: res.data.lendDetail.borrower, // 借款人信息
  ...
  // mounted()在客户端的浏览器中执行，可以获取到cookie
  mounted() {
    // 查询账户余额
    this.fetchAccount()
